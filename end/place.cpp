...
int SCPlace::SetArgCapacity(unsigned int capacity)
{
	...
	if(this->m_value > this->m_capacity)
		this->m_value = capacity;
...
int SCPlace::CommitTransTime()
{
	...
	double arraySize = 0;
	double val = 0;
	//ak existuju prechody s nulovym casom
	...
	while(ok)
	{
		...
		for(it = this->m_directedArcsTo.begin();it<this->m_directedArcsTo.end();it++)
		{
			...
			if(currentProb == 0)
			{
				...
				(*it)->GetTarget()->SetProbabCallback(currentProb);
			}
			probability += currentProb;
...
void SCPlace::AddTransToCommit(SCTransition *tr)
{
	this->m_possibleTrans.push_back(tr);
}
int SCPlace::Action(int code, int param)
{
	...
	if(code == ACTION_TAKE)
	{
		if(m_value - param >= 0)
		{
			...
			this->m_value -= param;
			ret = PLACE_TAKEN;
			vector<SCTransition*>::iterator it2;
			...
			while(!this->m_possibleTrans.empty())
			{
				g_eventCal.Insert(this->m_possibleTrans.back(),this->m_possibleTrans.back()->GetExactTime());
				this->m_possibleTrans.pop_back();
			}

			vector<SCDirectedArc*>::iterator it;
			for(it = this->m_directedArcsFrom.begin(); it< this->m_directedArcsFrom.end(); it++)
			{
				if(this->m_capacity - this->m_value >= (*it)->GetArgWeight())
				{
					if(!(*it)->GetStart()->GetDirectedArcsFrom()->empty())
					{
						g_eventCal.Insert((*(*it)->GetStart()->GetDirectedArcsFrom()->begin())->GetStart(),g_time);
					}
				}
			}
		}
	...
	{
		if(m_value + param <= this->m_capacity)
		{
			if( g_print == true && this->m_print == true )
				cout << "time: '" << g_time << "' --> " << this->m_name.c_str() << "(Act. value ['"<< this->m_value << "'";
