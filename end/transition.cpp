...
{
	if(this->m_probabCallback != 0 &&this->m_probability == 0)
	{
		return this->m_probabCallback;
	}
...
void SCTransition::SetProbabCallback(double probab)
{
	if(this->m_probabCallback != probab)
		this->m_probabCallback = probab;
}
...
{
	if(this->m_priority == 0 && this->m_time == 0 && probability <= 100 && probability >= 0)
	{
...
{
	if(this->m_priority == 0 && this->m_probability == 0 && time > 0)
	{
...
int SCTransition::SetArgTimeEx(double from,double to,int type)
{
...
bool SCTransition::IsReadyToRun()
{
	...
	if(status)
	{
		for(it=this->m_directedArcsTo.begin();it<this->m_directedArcsTo.end();it++)
		{
			...
			{
				(*it)->GetTarget()->AddTransToCommit(this);
				status = false;
				break;
			}
		}
...
double SCTransition::GetExactTime()
{
	...
	case TIME_NORM:
		...
		break;
	case TIME_ROV:
		time = g_time + SCGen::GenNomInterval(this->m_time,this->m_timeEnd);
		break;
	}
	return time;
}
	...
	if(IsReadyToRun())
	{
		if(this->m_status == TRANSITION_OK)
		{
			...
			for(it = this->m_directedArcsFrom.begin();it<this->m_directedArcsFrom.end();it++)
			{
				...
				this->m_totalIn += (*it)->GetArgWeight();
				if( g_printT == true )
			...
			for(it=this->m_directedArcsTo.begin();it<this->m_directedArcsTo.end();it++)
			{
				...
				this->m_totalOut += (*it)->GetArgWeight();
				if( g_printT == true )
