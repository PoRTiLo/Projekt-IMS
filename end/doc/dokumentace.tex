\documentclass[a4paper, 11pt]{report}[15.3.2010]									% velikost strany, styl a pismo
...
\end{itemize}
Pro úèely øe¹ení tohoto projektu nebyl vyu¾it ¾ádný pravidelný poradce, tudí¾ nebylo pou¾ito ¾ádných rad z~tøetích stran a¾ na jednu konzultaci s~hodnotícím, panem Hrubým Martinem, Ing., Ph.D..
...
\begin{description}
  ...
  \item[pravdìpodobnosti pøechodù] - (\href {https://www.fit.vutbr.cz/study/courses/index.php?id=7409}{odkaz na pøedmìt}, slajd 133) je udávána v~procentech. Celkový souèet \newline pravdìpodobnosti v¹ech pøechodù ve stejné úrovni musí být \textit{1}, neboli \textit{100\%}.
...
\end{description}
Tato roz¹íøení se navzájem nesmí kombinovat (\href {https://www.fit.vutbr.cz/study/courses/index.php?id=7409}{odkaz na pøedmìt}, slajd 134). Obrázek \ref{pic4} ukazuje pou¾ití P/T s~roz¹íøeními.
...
%---------------------------------------------------
    ...
    \node at ( 7,0) [transition] (t1)[fill=gray!20,label=below:3s]{}
      edge [pre] (p0)
      edge [post] (p1);
%-----------------------------------------------------
...
\section{Tvorba simulátoru}
Simulátor byl navr¾ený tak, aby uspokojoval nejzákladnìj¹í parametry vyu¾ití a~byl jednoduchý a~srozumitelný pro u¾ivatele. Základní cíle byly tedy stanoveny na jednoduchost a~intuitivnost. S~trochou pøedstavivosti je pøedvídatelné co ka¾dá zadaná funkce dìlá a~jaké je její pou¾ití. Na výsledný simulátor by bylo pomìrnì jednoduché navázat grafické u¾ivatelské rozhraní. Dal¹ím významným kriteriem pøi tvorbì, byla nutnost eliminovat co nejvíce chyb pøi samotném zápise zkoumaného modelu. Pøi prvním pohledu na slo¾itìj¹í model, mù¾e být jeho význam nejasný, ale po podrobnìj¹ím prostudování je koncepènì toto¾ný s~grafickým zápisem. Samotný nápad o~zápis modelu a~jeho ovládání je výhradnì dílem autora, inspirovaný podobnou knihovnou SIMLIB (\href {https://www.fit.vutbr.cz/study/courses/index.php?id=7409}{odkaz na pøedmìt}, slajd 167).

\section{Podnìty tvorby}
PetriSim 1.0.0 byl vytvoøený jen z~poznatkù a~nápadù autorù, bez ovlivnìní studováním funkènosti jiných podobných nástrojù pro simulaci. Pou¾ité algoritmy nejen, ¾e umo¾òují zápis a~simulaci Petriho sítì s~roz¹íøeními definovanými v~úvodu, ale i~sémantickou kontrolu správnosti výsledného modelu a~schopnost adekvátní simulace. Bez této kontroly by byl napøíklad mo¾ný nekoneèný cyklus v~nulovém èase, a~nebo prùbìh nekoneènì trvající simulace. V~pøípadì takovýchto zji¹tìní, je u¾ivatel okam¾itì upozornìný o~vzniklé situaci a~je mu navrhnuto øe¹ení. Nejvìt¹í motivací pro nekopírování nápovìd byla tu¾ba vymyslet nìco originálního. Autoøi pøi zaèátku tvorby ji¾ mìli zku¹enosti s~danou problematikou.

\chapter{Koncepce simulátoru}

Simulátor je postavený na konceptu jednoduchosti a~intuitivnosti, aby se co nejvíce pøiblí¾il logikou grafickému zápisu a~byl tak pochopitelný pro u¾ivatele. Skládá se z~tøech hlavních tøíd a~to \texttt{SCTransition}, \texttt{SCPlace} a~\texttt{SCDirectedArc}, pøièem¾ ka¾dá z~nich reprezentuje urèitou èást grafického zápisu. Tøída \texttt{SCTransition} pøechod, tøída \texttt{SCPlace} místo a~tøída \texttt{SCDirectedArc} hranu. Ka¾dé tøídì je mo¾né pøidìlit nastavení podle definice Petriho sítì. Podobnì jako pøi grafickém zápise, je na vytvoøení koneèného modelu potøebné propojit v¹echny elementy. Podle definice je mo¾né spojit pøechod s~místem, na co¾ slou¾í funkce \texttt{TrToPl(pøechod,míto,hrana)}, zkratka \uv{TransitionToPlace}, a~nebo místo s~pøechodem pomocí funkce \texttt{PlToTr(místo,pøechod,hrana)}, zkratka \uv{PlaceToTransition}. Konkrétní funkènost, pøesné mo¾nosti nastavení jednotlivých elementù, jako i~nastavení výpisu statistik a~provedení simulace budou popsány v~dal¹ích kapitolách.

\chapter{Architektura simulaèního modelu/simulátoru}
Pro samotné pou¾ití knihovny je nutné ve zdrojovém souboru jazyka C++ pøidat hlavièkový soubor \texttt{PetriSim.h}, který plnì zpøístupní danou knihovnu. Na výbìr je statická a~nebo dynamická knihovna, kterou je nutné pøilinkovat pøi kompilaci výsledného programu.
\section{Tøídy}
\subsection*{SCBase}
...
\multicolumn{3}{l}{\texttt{int\textbf{ SetArgPrio(}unsigned int prio\textbf{)}}} \\
  ...
  & funkènost                & nastaví pravdìpodobnost danému pøechodu\\
  & vrácená hodnota          & ¾ádná\\ \hline
  \end{tabular}
  %\caption{Srovnání slu¾eb WWW, Email a Jabber}
  %\label{tab2}
...
\subsection*{SCStats}
Tøída starající se o~statistiky simulace. Výpis statistiky závisí na pou¾ití funkcí \texttt{PrintSteps(bool stat)} a~\texttt{PrintSummary(bool stat)}. Poèáteèní hodnota tisku statistik se nachází ve stavu \uv{tiskni}. Pokud je pøíznak tisku nastaven, statistiky se tisknou na standardní výstup. Funkce \texttt{PrintSteps} ovládá tisk výpisù po jednotlivých krocích, tiskne se tedy pokud probìhne v~simulaci zmìna. Druhá funkce \texttt{PrintSummary} se stará o~tisk souhrnné výsledné statistiky.

Pomocí privátní promìnné tøídy \texttt{CSPlace} lze mìnit tisk statistiky po krocích pro jednotlivá místa. Tato promìnná je pro ka¾dé místo nastavena na true, její hodnotu mù¾eme zmìnit funkcí \texttt{SetArgPrint( bool stav)}.
...
\multicolumn{3}{l}{\texttt{int\textbf{ PrintSummary(}bool stat\textbf{)}}} \\
  & parametry                & zapnutí/vypnutí statistiky - \texttt{bool}\\
  & funkènost                & zapne nebo vypne vypisování koneèné souhrnné statistiky\\
  & vrácená hodnota          & ¾ádná\\ \hline
\multicolumn{3}{l}{\texttt{int\textbf{ PrintSteps(}bool stat\textbf{)}}} \\
  & parametry                & zapnutí/vypnutí statistiky - \texttt{bool}\\
  & funkènost                & zapne nebo vypne vypisování statistiky po kroku simulace\\
  & vrácená hodnota          & ¾ádná\\ \hline
\multicolumn{3}{l}{\texttt{int\textbf{ ClearSimulation()}}} \\
  & parametry                & ¾ádné\\
  & funkènost                & provede vynulování statistiky\\
  & vrácená hodnota          & ¾ádná\\ \hline
  \end{tabular}
  %\caption{Srovnání slu¾eb WWW, Email a Jabber}
  %\label{tab2}
...
\begin{table}[h]
\begin{tabular}{ l l }
\texttt{PrintSteps(true)} & // nastavení tisku statistiky po krocích\\
\texttt{PrintSummary(false)} & // vypnutí tisku celkové souhrnné statistiky\\\newline
...
\begin{center}
  \begin{tikzpicture}
    ...
    \node at ( 3,1) [place, label=above:$p_1$] (p2){};
    \node at ( 3,-1) [place, label=above:$p_2$] (p3){};
    \node at ( 1,1) [transition] (t1)[fill=gray!20,label=below:10-20]{}
      edge [pre] (p0)
      edge [post] (p2);
    \node at (1,-1) [transition] (t2){}
      edge [pre] (p1)
...
\begin{center}
  \begin{tikzpicture}
    ...
    post/.style={->, shorten >=1pt, >=stealth', semithick}]
    \node at ( 1,1) [place, label=above:$p_0$] (p0){};
    \node at ( -1,1) [transition] (t1)[fill=gray!20,label=below:exp(5)]{}
      edge [post] (p0);
  \end{tikzpicture}
  \caption{Model reprezentující èasovaný pøechod}
  \label{mod3}
...
\newpage
\section{Model}
Na obrázku \ref{mod5}  je vyobrazen komplexní model Petriho sítì. Tato sí» byla vytvoøena za úèelem ukázat mo¾nosti implementované knihovny. Tudí¾ nevychází z~reality. Spojuje zde v¹echny po¾adované vlastnosti míst, pøechodù i~hran.
\begin{figure}[h]
\begin{center}
\scalebox{0.85}{
...
Obrázek \ref{stat2} pojednává o~celkové souhrnné statistice. \texttt{Place count} znaèí celkový poèet míst v~zadaném modelu, \texttt{Transition count} celkový poèet pøechodù v~modelu a~\texttt{Directeds count} celkový poèet hran v~modelu. \texttt{Simulation time} je èas bìhu simulace.
\begin{figure}[h]
$\texttt{************************ STATISTICS **********************}$\\
\texttt{* \hspace{1cm}       T}\\
...
\caption{Pøíklad souhrnné statistiky}
\label{stat2}
\end{figure}
